...
export const PortfolioDashboard: React.FC = () => {
    ...
    const fetchAllData = async () => {
        try {
            const [profileRes, holdingsRes, positionsRes, ordersRes, marginsRes, gttRes] = await Promise.all([
                ...
                fetch('http://127.0.0.1:8001/api/portfolio/gtt')
            ]);
    ...
    const calculatePnL = (holding: Holding): number => {
        // Use backend-provided P&L or calculate from prices
        if (holding.pnl !== undefined && holding.pnl !== null) {
            return holding.pnl;
        }
    ...
    const getPnLColor = (holding: Holding): string => {
        const pnl = calculatePnL(holding);
        return pnl >= 0 ? 'text-emerald-500' : 'text-red-500';
    };

    const totalHoldingsValue = holdings.reduce((sum, h) => sum + ((h.last_price || 0) * (h.quantity || 0)), 0);
    const totalHoldingsPnL = holdings.reduce((sum, h) => sum + calculatePnL(h), 0);
    const totalPositionsPnL = [...positions.net, ...positions.day].reduce((sum, p) => sum + (p.pnl ?? 0), 0);
    ...
    return (
        <div className="space-y-6">
            ...
            {/* User Profile Card */}
            {profile && (
                <div className="bg-gradient-to-br from-blue-900/20 to-purple-900/20 border border-zinc-800 rounded-xl p-6">
                        ...
                        <div className="flex-1">
                            ...
                            <div className="flex items-center gap-4 mt-2">
                                ...
                                <span className="text-xs text-zinc-500">
                                    Exchanges: {profile.exchanges?.join(', ') || 'N/A'}
                                </span>
                            </div>
                        </div>
                    </div>
                            ...
                            <tbody className="divide-y divide-zinc-800">
                                {holdings.map((holding, idx) => (
                                    <tr key={idx} className="hover:bg-zinc-800/50">
                                        ...
                                        <td className={`px-6 py-4 text-right font-semibold ${getPnLColor(holding)}`}>
                                            {formatCurrency(calculatePnL(holding))}
                                        </td>
                            ...
                            <tbody className="divide-y divide-zinc-800">
                                {orders.slice(0, 10).map((order: any) => (
                                    <tr key={order.order_id} className="hover:bg-zinc-800/50">
                                        ...
                                        <td className="px-6 py-4 text-center">
                                            <span className={`text-xs px-2 py-1 rounded ${order.transaction_type === 'BUY' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'
                                                }`}>
                                        ...
                                        <td className="px-6 py-4 text-center">
                                            <span className={`text-xs px-2 py-1 rounded ${order.status === 'COMPLETE' ? 'bg-emerald-500/20 text-emerald-400' :
                        ...
                        {gttOrders.map((gtt) => (
                            <div key={gtt.id} className="bg-zinc-800/50 border border-zinc-700 rounded-lg p-4">
                                <div className="flex items-center justify-between mb-2">
                                    ...
                                    <span className={`text-xs px-2 py-1 rounded ${gtt.status === 'active' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-zinc-700 text-zinc-400'
                                        }`}>
                                        {gtt.status}
                                    </span>
                                </div>
