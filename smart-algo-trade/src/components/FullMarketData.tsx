...
export const FullMarketData: React.FC<FullMarketDataProps> = ({
    ...
    const fetchQuotes = async () => {
        try {
            ...
            if (data.status === 'success' && data.data) {
                ...
                console.warn('No quote data available. Market may be closed or symbols invalid.');
                setQuotes({});
            }
        } catch (error) {
    ...
    useEffect(() => {
        ...
        if (autoRefresh) {
            const interval = setInterval(fetchQuotes, 1000); // Update every 1 second
    ...
    return (
        <div className="space-y-6">
            ...
            {Object.keys(quotes).length === 0 && (
                <div className="bg-zinc-900 border border-yellow-900/50 rounded-lg p-8 text-center">
                    <Activity className="w-12 h-12 text-yellow-500 mx-auto mb-4 opacity-50" />
                    <h3 className="text-lg font-semibold text-zinc-300 mb-2">No Market Data Available</h3>
                    <p className="text-zinc-500 mb-4">
                        This usually means the market is closed. Stock market in India is open Monday to Friday, 9:15 AM to 3:30 PM IST.
                    </p>
                    <p className="text-sm text-zinc-600">
                        Current Time (IST): {new Date(new Date().getTime() + 5.5 * 60 * 60 * 1000).toLocaleTimeString('en-IN')}
                    </p>
                    <p className="text-sm text-zinc-600 mt-2">
                        Try using the quote endpoint with: NSE:NIFTY50-INDEX, NSE:BANKNIFTY25-INDEX, or other active symbols when market is open.
                    </p>
                </div>
            )}
                        ...
                        <div className="bg-gradient-to-r from-zinc-900 to-zinc-950 p-6 border-b border-zinc-800">
                            <div className="flex items-start justify-between">
                                ...
                                <div className="text-right">
                                    ...
                                    <div className={`flex items-center gap-1 justify-end mt-1 ${isPositive ? 'text-emerald-500' : 'text-red-500'
                                        }`}>
